...
export default {
    ...
    components: {ButtonUi},
    layout: Layout,
    props: ['client', 'contacts', 'stores', 'types', 'errors'],
    data() {
        return {
            form: {
    ...
    methods: {
        ...
        addContact() {
            axios.post(this.route('contacts.store', this.client.id), {
                type: this.contact.type,
                value: this.contact.value
            }).then(response => {
                this.form.contacts.push(response.data.data);
                this.contact.type = '';
                this.contact.value = '';
            });
        },
        removeContact(index, id) {
            axios.delete(this.route('contacts.delete', id)).then(() => {
                                ...
                                <tr class="bg-white" v-for="(contact, index) in form.contacts" :key="contact.id">
                                    ...
                                    <td class="py-4 text-right">
                                        <button type="button" class="text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 rounded-lg font-medium text-xs p-2.5 text-center inline-flex items-center me-2"
                                                @click.prevent="removeContact(index, contact.id)"
                                        >
